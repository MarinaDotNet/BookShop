...
| API: | Description: | API Version: | Additional Notes: |
| ---- | ------------ | ------------ | ----------------- |
| <span id="GET/books/all" name="GET/books/all">GET/books/all</span> | Retrieves all data from database. | ApiVersion-BooksStore: 1<br>ApiVersion-BooksStore: 2 | For ApiVersion-BooksStore : 2 retrieves only were parameter “isAvailable” : true |
...
| GET/books/available/page | Returns list of products where parameter isAvailable equals requested true or false, at requested page with requested quantity. | ApiVersion-BooksStore: 1 | Deviding content into pages as in: [GET/books/all](#GET/books/all) method |
...
| GET/books/genre | Returns a list of products under requested genre, on requested page and requested quantity | ApiVersion-BooksStore: 1<br>ApiVersion-BooksStore: 2 | Devides content into pages, returns requested page simmular to method: [GET/books/all](#GET/books/all).<br>For ApiVersions 2 and 3 returns genres where at least one product isAvailable == true. |
| GET/books/filter/condition/equals | Returns list of ALL products where Title, Author, Language or one of item from array of Genres EQUALS to searchCondition.| ApiVersion-BooksStore: 1<br>ApiVersion-BooksStore: 2 | Devides content into pages, returns requested page simmular to method: [GET/books/all](#GET/books/all).<br>For ApiVersion: 2 returns only products where isAvailable == true. |
| GET/books/filter/condition/contains | Returns list of ALL products where Title, Author, Language or one of item from array of Genres CONTAINS the searchCondition.| ApiVersion-BooksStore: 1<br>ApiVersion-BooksStore: 2 | Devides content into pages, returns requested page simmular to method: [GET/books/all](#GET/books/all).<br>For ApiVersion: 2 returns only products where isAvailable == true. |
| GET/books/filter | Sorts the list of all products by requested parameters. For each specified parameter that contains data. | ApiVersion-BooksStore: 1<br>ApiVersion-BooksStore: 2<br>ApiVersion-BooksStore: 3 | Filters the list by each provided parameter. Exception if IsAvailable was not specified, then its sets as false for ApiVersion 1, and always sets to true for ApiVersions 2 and 3. Devides content into pages, returns requested page simmular to method: [GET/books/all](#GET/books/all). |
